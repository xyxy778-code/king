...
| 用途 | 说明 |
|------|------|
| 设计 | 使用 AI 讨论架构、类设计、接口定义 |
| 调试 | 将编译错误 / 运行异常提供给 AI，定位修复 |
| 解释 | 让 AI 解释 Java 语法、OOP 概念、代码逻辑 |
| 测试 | 让 AI 生成测试用例，验证功能正确性 |
| 重构 | 要求 AI 按 OOP 原则优化代码结构 |
| 文档 | 使用 AI 辅助生成 plan.md、注释等文档 |

### 12.2 禁止的行为

| 行为 | 说明 |
|------|------|
| 不理解代码 | 不允许直接使用 AI 生成的代码而不理解其逻辑 |
| 隐藏 AI 使用 | 所有 AI 辅助过程必须记录在 DevLog 中 |
| 伪造 Git 记录 | 禁止篡改 git commit 时间、作者等信息 |
| 抄袭他人代码 | 禁止将第三方代码作为自己的提交 |
| 捏造测试结果 | 测试必须实际运行，禁止伪造 通过/失败 |

### 12.3 合规检查清单

每次提交前检查：
- [ ] 代码是否已理解（能解释每段逻辑）
- [ ] DevLog 是否已记录对应的 AI 交互
- [ ] Git 记录是否真实（commit 时间、作者）
- [ ] 代码是否为独立完成（无第三方抄袭）
- [ ] 测试结果是否为实际运行输出
